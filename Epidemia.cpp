...
int main() {
    int Pop, Inf, Dead, Heal, Duration, Immunity, VStart, VMax, NewSusc;
    double Beta, Gamma, HIndex, VIndex;  // so che Giacomini non apprezza
    char Already;
...
/*  
    ...
    std::cin >> VMax;
    while ( VMax <= 0 ) {
        std::cout << " VMax has to be > than 0.\n ";
        std::cin >> VMax;
    }
    ...
    std::cin >> Already;
    while ( Already != 'N' && Already != 'Y' && Already  != 'n' && Already != 'y' ) {
        std::cout << " Already has to be Y/n.\n ";
        std::cin >> Already;
    }
...
*/
    ...
    Data initial_state  {Pop - Inf - Dead - Heal, Inf, Dead, Heal, Dead + Heal, Immunity, VStart, VMax, 
                        NewSusc, Beta, Gamma, HIndex, VIndex, Already};  // aggiungere c
    ...
    std::cout << '\n' << std::setw(12) << "Giorno   " << "|" << std::setw(12) << "Suscettibili" << "|" 
              ...
              << std::setw(12) << "Guarigios." << "|" << std::setw(12) << "Popolation" << "|\n"
              << "--------------------------------------------------------------------------------------------------------" << '\n';
            ...
            while (d < Duration+1) {
                ...
                if (d > Duration && d != Duration + 10) {
                    control_print   ( Duration, data[Duration].Susc_, data[Duration].Inf_, data[Duration].Recov_,
                                    data[Duration].Heal_, data[Duration].Dead_, data[Duration].HIndex_, Pop );
                }
            }
            break;
    ...
    std::cout << "La simulazione termina il " << ldt->tm_mday << ' ' << ConvertToString(ldt->tm_mon) << ' '
